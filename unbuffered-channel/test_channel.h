...
void CheckValues(const std::vector<std::vector<int>>& send_values,
    ...
    std::vector<int> send_not_recv{};
    std::vector<int> recv_not_send{};
    std::ranges::set_difference(all_send, all_recv, std::back_inserter(send_not_recv));
    std::ranges::set_difference(all_recv, all_send, std::back_inserter(recv_not_send));
    REQUIRE(send_not_recv.empty());
    REQUIRE(recv_not_send.empty());
